...
*
* @global object $GLOBALS['config']
* @param integer $id tmdb genre id
* @param integer $page page
* @return object
*/
...
*
* @global object $GLOBALS['config']
* @param integer $q query keywords
* @param integer $page page
* @return object
*/
...
*
* @global object $GLOBALS['config']
* @param integer $id tmdb title id
* @param integer $page page
* @return object
*/
 ...
 * @global object $GLOBALS['config']
 * @param  integer $id tmdb title id
 * @param  integer $season_number season number
 * @return object
 */
 ...
 * @param  integer $id tmdb title id
 * @param  integer $season_number season number
 * @param  integer $episode_number episode number
 * @return object
 */
 ...
 *
 * @global object $GLOBALS['config']
 * @param integer $id tmdb person id
 * @return object
 */
...
* @param integer $hours cache time in hours
		...
		if(file_exists($file) && ($current_time - $expire_time < $file_time)) {
			if ($config->memcache && $memcacheConnected){
				...
				if ($output){
					...
					if (is_json($output)) {
						...
						$output = array('error'=>'file cache error','error_message'=>'error file_get_contents file cache : ' . $key);
			...
			if (is_json($content)) {
				...
				if (!isset($cek->status_code)) {	
					...
					$output = array('error'=>'respon tmdb api error');
					if (empty($content)){
				...
				$output = array('error'=>'crawl error');
				if (empty($content)){
					$output['error_message'] = 'content kosong!';
				} else {
